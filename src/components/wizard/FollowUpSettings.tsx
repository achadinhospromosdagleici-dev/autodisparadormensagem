...
export interface FollowUpConfig {
  enabled: boolean;
  mode: FollowUpMode;
  greetingMessageIndex: number;
  waitForReplyTimeout: number;
  maxRetries: number;
        ...
        <div className="space-y-4 animate-fade-in">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Envia a saudação, aguarda resposta, depois envia todas as mensagens restantes automaticamente.
                    </p>
                  </div>
                </div>
              </button>
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Envia a saudação, aguarda resposta, envia a próxima, aguarda novamente, e assim por diante.
                    </p>
                  </div>
                </div>
              </button>
          ...
          <div className="glass-card p-6 space-y-3">
            ...
            <div className="flex flex-col gap-2">
              ...
              <FlowArrow />
              <FlowStep icon={<Clock className="w-4 h-4" />} label={`Aguardar resposta (ate ${config.waitForReplyTimeout}min)`} color="text-warning" />
              <FlowArrow />
              <FlowStep icon={<CheckCircle2 className="w-4 h-4" />} label="Resposta recebida" color="text-success" />
              <FlowArrow />
              <FlowStep
                icon={<MessageSquare className="w-4 h-4" />}
                label={config.mode === 'greeting-then-all' ? 'Enviar todas as mensagens restantes' : 'Enviar proxima mensagem e aguardar'}
                color="text-primary"
              />
              {config.maxRetries > 0 && (
                <>
...
function FlowStep({ icon, label, color }: { icon: React.ReactNode; label: string; color: string }) {
  return React.createElement('div', { className: `flex items-center gap-3 p-3 rounded-lg bg-muted/30 border border-border/30 ${color}` }, icon, React.createElement('span', { className: 'text-sm' }, label));
}

function FlowArrow({ dashed }: { dashed?: boolean }) {
  return React.createElement('div', { className: 'flex justify-center' },
    React.createElement('div', { className: `w-px h-6 ${dashed ? 'border-l-2 border-dashed border-muted-foreground/30' : 'bg-border'}` })
  );
}