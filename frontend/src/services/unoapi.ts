...
async function loadUnoApiFromDb(): Promise<UnoApiCredentials | null> {
  try {
    ...
    const response = await api.get('/settings/unoapi');
    if (!response.data) return null;
    const data = response.data;
    return {
      baseUrl: data.baseUrl || data.base_url,
      token: data.token,
      s3Enabled: data.s3Enabled || data.s3_enabled,
      s3Endpoint: data.s3Endpoint || data.s3_endpoint,
      s3AccessKey: data.s3AccessKey || data.s3_access_key,
      s3SecretKey: data.s3SecretKey || data.s3_secret_key,
      s3Bucket: data.s3Bucket || data.s3_bucket,
      s3Region: data.s3Region || data.s3_region,
    };
  } catch (error) {
...
async function proxyCall(creds: UnoApiCredentials, endpoint: string, method = 'GET', requestBody?: any): Promise<{ ok: boolean; data: any }> {
  try {
    ...
    console.error('[unoapi] proxyCall error:', err.response?.status, err.response?.data || err.message);
...
async function detectEvolutionApi(creds: UnoApiCredentials): Promise<boolean> {
  try {
    const response = await api.post('/proxy/evolution', { action: 'fetchInstances', baseUrl: creds.baseUrl, apiKey: creds.token });
    return response.data && Array.isArray(response.data.instances);
  } catch {
    return false;
  }
}
